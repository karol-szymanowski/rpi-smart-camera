...
function drawFaces(canvas, data, fps) {
    ...
    ctx.fillStyle = 'white';
    ctx.fillText(`FPS: ${fps}`, 40, 25);
    if (temp !== null) {
        ctx.fillText(`CPU: ${temp}℃`, 40, 50);
    }
    for (const person of data) {
        const x = canvas.width - person.detection.box.x - person.detection.box.width
        ...
        ctx.globalAlpha = 0.6;
        ctx.beginPath();
        ctx.rect(x, person.detection.box.y, person.detection.box.width, person.detection.box.height);
        ctx.stroke();
        ctx.globalAlpha = 1;
        // const expression = person.expressions.sort((a, b) => Object.values(a)[0] - Object.values(b)[0]);
        // const expression = Object.entries(person.expressions).sort((a, b) => b[1] - a[1]);
        ctx.fillStyle = 'black';
        ctx.fillText(`gender: ${Math.round(100 * person.genderProbability)}% ${person.gender}`, x, person.detection.box.y - 41);
        // ctx.fillText(`expression: ${Math.round(100 * expression[0][1])}% ${expression[0][0]}`, person.detection.box.x, person.detection.box.y - 41);
        ctx.fillText(`age: ${Math.round(person.age)} years`, x, person.detection.box.y - 23);
        ...
        ctx.fillText(`gender: ${Math.round(100 * person.genderProbability)}% ${person.gender}`, x, person.detection.box.y - 42);
        // ctx.fillText(`expression: ${Math.round(100 * expression[0][1])}% ${expression[0][0]}`, person.detection.box.x, person.detection.box.y - 42);
        ctx.fillText(`age: ${Math.round(person.age)} years`, x, person.detection.box.y - 24);
